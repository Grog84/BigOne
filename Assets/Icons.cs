...
public class Icons : MonoBehaviour
{
	...
	void Update ()
    {
        ...
        else if (GMController.instance.isCharacterPlaying == controllerMother.thisCharacter)
        {
            ...
            // Collect Icon
            if (controllerMother.m_CharacterController.isInItemArea && !controllerMother.m_CharacterController.isPushDirectionRight && !controllerMother.m_CharacterController.isDoorDirectionRight)
            {
                gameObject.transform.Find("Key").gameObject.SetActive(true);
            }
            else
