...
namespace FMODUnity
{
    [CustomPropertyDrawer(typeof(MultipleFMOD))]
    class MultipleEventRefDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (e.type == EventType.dragPerform && position.Contains(e.mousePosition))
            {
                if (DragAndDrop.objectReferences.Length > 0 &&
                    ...
                    DragAndDrop.objectReferences[0].GetType() == typeof(EditorMultipleFMOD))
                {
                    pathProperty.stringValue = ((EditorMultipleFMOD)DragAndDrop.objectReferences[0]).Path;
                    GUI.changed = true;
                    e.Use();
                }
            }
            ...
            if (!String.IsNullOrEmpty(pathProperty.stringValue) && EventManager.EventFromPath(pathProperty.stringValue) != null)
            {
                ...
                if (property.isExpanded)
                {
                    ...
                    style.richText = true;
                    EditorEventRef eventRef = EventManager.EventFromPath(pathProperty.stringValue);  // this is single
